"""
COMP 163 - Project 2: Character Abilities Showcase
Name: [DeAundre Black]
Date: [11-14-2025]

AI Usage: I used ChatGPT to implement classes (hierarchy and etc) as well as clean up any errors in my code(syntax and operators.)


# ============================================================================
# PROVIDED BATTLE SYSTEM (DO NOT MODIFY)
# ============================================================================

class SimpleBattle:
""
    Simple battle system provided by the instructor.
    Do NOT change this class. It is only used to test character behavior.
    """

    def __init__(self, character1, character2):
        self.char1 = character1
        self.char2 = character2

    def fight(self):
        """Runs a simple one-round battle between two characters."""
        print(f"\n=== BATTLE: {self.char1.name} vs {self.char2.name} ===")

        # Show stats before fighting
        print("\nStarting Stats:")
        self.char1.display_stats()
        self.char2.display_stats()

        print(f"\n--- Round 1 ---")
        print(f"{self.char1.name} attacks:")
        self.char1.attack(self.char2)

        # Second character only attacks if still alive
        if self.char2.health > 0:
            print(f"\n{self.char2.name} attacks:")
            self.char2.attack(self.char1)

        print(f"\n--- Battle Results ---")
        self.char1.display_stats()
        self.char2.display_stats()

        # Determine the winner
        if self.char1.health > self.char2.health:
            print(f"🏆 {self.char1.name} wins!")
        elif self.char2.health > self.char1.health:
            print(f"🏆 {self.char2.name} wins!")
        else:
            print("🤝 It's a tie!")


# ============================================================================
# CHARACTER CLASSES TO IMPLEMENT
# ============================================================================

import random  # Needed for Rogue critical chance


class Character:
    """
    Base class for all characters.
    All players and enemies inherit these attributes and methods.
    """

    def __init__(self, name, health, strength, magic):
        self.name = name
        self.health = health
        self.strength = strength
        self.magic = magic

    def attack(self, target):
        """Basic attack based only on the character's strength."""
        damage = self.strength
        target.take_damage(damage)

    def take_damage(self, damage):
        """Reduces health without letting it drop below zero."""
        self.health -= damage
        if self.health <= 0:
            self.health = 0

        if self.health == 0:
            print("Character Defeated!")

    def display_stats(self):
        """Prints the character's current stats in a simple format."""
        print(f"Character Name: {self.name}")
        print(f"Current Health: {self.health}")
        print(f"Current Strength: {self.strength}")
        print(f"Current Magic: {self.magic}")


class Player(Character):
    """
    Parent class for all playable character types.
    Adds class name, level, and experience tracking.
    """

    def __init__(self, name, character_class, health, strength, magic, current_level=1, exp=100):
        super().__init__(name, health, strength, magic)
        self.character_class = character_class
        self.current_level = current_level
        self.exp = exp
        self.level = current_level

    def display_stats(self):
        """Shows base stats plus the player's class, level, and experience."""
        super().display_stats()
        print(f"Character Class: {self.character_class}")
        print(f"Character's Current Level: {self.current_level}")
        print(f"Character's Experience Level: {self.exp}")


class Warrior(Player):
    """
    Warrior class: strong physical fighter with high strength and health.
    """

    def __init__(self, name):
        super().__init__(name=name, character_class="Warrior", health=120, strength=20, magic=5)

    def attack(self, target):
        """
        Warrior-specific attack.
        Uses some strength and deals stronger physical damage.
        """
        if self.strength >= 4:
            self.strength -= 4
            damage = self.strength * 0.7
            target.health -= damage
        else:
            print("Low strength, not enough to attack")

    def power_strike(self, target):
        """
        Warrior special ability.
        Costs more strength but deals a heavy damage boost.
        """
        if self.strength >= 7:
            self.strength -= 7
            damage = self.strength * 1.75
            target.health -= damage
        else:
            print("Too much strength used! Not enough left to use power strike")


class Mage(Player):
    """
    Mage class: specializes in magic attacks, low strength and health.
    """

    def __init__(self, name):
        super().__init__(name, character_class="Mage", health=85, strength=10, magic=25)

    def attack(self, target):
        """
        Mage attack that uses magic instead of strength.
        Costs a small amount of magic to cast.
        """
        if self.magic >= 3:
            self.magic -= 3
            damage = self.magic * 0.4
            target.health -= damage
        else:
            print("Not enough magic left to attack")

    def fireball(self, target):
        """
        Mage special ability.
        A higher-cost spell that deals strong magic damage.
        """
        if self.magic >= 6:
            self.magic -= 6
            damage = self.magic * 1.2
            target.health -= damage
        else:
            print("Not enough magic to use fireball")


class Rogue(Player):
    """
    Rogue class: fast attacker with a chance to land critical hits.
    """

    def __init__(self, name):
        super().__init__(name=name, character_class="Rogue", health=90, strength=15, magic=15)

    def attack(self, target):
        """
        Rogue normal attack with a 30% chance to deal double damage.
        """
        basic_attack = self.strength * 0.75
        critical_chance = random.randint(1, 10)

        if critical_chance <= 3:
            damage = basic_attack * 2
            print("Critical Hit!")
        else:
            damage = basic_attack

        target.health -= damage

    def sneak_attack(self, target):
        """
        Rogue special ability: guaranteed critical strike.
        """
        base_attack = self.strength * 0.7
        sneak_damage = base_attack * 2
        print("Sneak attack hit!")
        target.health -= sneak_damage


class Weapon:
    """
    Represents a weapon that a character can hold.
    Provides a flat damage increase.
    """

    def __init__(self, name, damage_bonus):
        self.name = name
        self.damage_bonus = damage_bonus

    def display_info(self):
        """Prints the weapon's name and its damage bonus."""
        print(f"Weapon Name: {self.name}")
        print(f"Damage Bonus: {self.damage_bonus}")


# ============================================================================
# MAIN PROGRAM FOR TESTING
# ============================================================================

if __name__ == "__main__":
    print("=== CHARACTER ABILITIES SHOWCASE ===")
    print("Testing inheritance, polymorphism, and method overriding")
    print("=" * 50)

    # Create character examples
    warrior = Warrior("Sir Galahad")
    mage = Mage("T DA WIZARD")
    rogue = Rogue("Robin Hood")

    print("\nCharacter Stats:")
    warrior.display_stats()
    mage.display_stats()
    rogue.display_stats()

    print("\nTesting Polymorphism (each character uses attack differently):")
    dummy_target = Character("Target Dummy", 100, 0, 0)

    for character in [warrior, mage, rogue]:
        character.attack(dummy_target)
        dummy_target.health = 100  # Reset dummy

    print("\nTesting Special Abilities")
    target1 = Character("Enemy1", 50, 0, 0)
    target2 = Character("Enemy2", 50, 0, 0)
    target3 = Character("Enemy3", 50, 0, 0)

    warrior.power_strike(target1)
    mage.fireball(target2)
    rogue.sneak_attack(target3)

    sword = Weapon("Iron Sword", 10)
    staff = Weapon("Magic Staff", 15)
    dagger = Weapon("Steel Dagger", 8)

    sword.display_info()
    staff.display_info()
    dagger.display_info()

    print("\nTesting Battle System:")
    battle = SimpleBattle(warrior, mage)
    battle.fight()

    print("\n✅ Testing complete!")
